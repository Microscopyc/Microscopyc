...
def results_pandas(model, test_loader):
    ...
    for result in results:
        ...
        results_dict["Top 1 Correct"].append(
            1 if result.get_gt() == result.get_top5_classes()[0] else 0
        )
        results_dict["Top 5 Correct"].append(
            1 if result.get_gt() in result.get_top5_classes() else 0
        )
